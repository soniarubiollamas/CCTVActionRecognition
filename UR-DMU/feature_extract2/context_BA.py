...
def select_majority_normal_features(features, percentage, name):
    ...
    for i in range(features.shape[0]):
        ...
        extended_sequence = np.concatenate((best_segment, features[i], best_segment), axis=0)
...
def process_all_videos(directory, percentage):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".npy"):
            ...
            extended_frames = select_majority_normal_features(features, percentage, name)
...
directory = "UCF_ten/30fps/test/repeat" 
percentage = 0.2
