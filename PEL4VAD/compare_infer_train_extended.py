...
def plot_npy_files_gt(folder1, folder2, folder4, filename, label1, label2, label4, ax):
    ...
    """""  to represent the whole context """""

    # max_len = max(len(data1), len(data2))
    # data1 = np.append(np.repeat(0, len(data2) - len(gt)), data1)
    # if max_len < len(gt):
    #     # truncar gt
    #     gt = gt[:len(data2)]
    ...
    # if max_len > len(gt):
    #     gt_temp = gt[0]
    #     gt = np.append(np.repeat(gt_temp, len(data2) - len(gt)), gt)


    """""  only represent the real video """""

    max_len = max(len(data1), len(gt))
    context_len = (len(data2) - len(gt))
    data2 = data2[context_len:]
...
for i, archivo in enumerate(archivos_seleccionados):
    ...
    PEL4VAD = f'predictions/{mode}/30fps/{filename}'
    URDMU = f'predictions/{mode}/30fps/extended_20/{filename}'
