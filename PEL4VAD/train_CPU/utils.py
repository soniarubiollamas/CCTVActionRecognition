...
def get_cas(x_v, x_t, logits, labels, scale=10):
    ...
    video_feat = torch.zeros(0).tensor([]) #.cuda()  # tensor([])
    token_feat = torch.zeros(0).tensor([]) #.cuda()  # tensor([])
    video_labels = torch.zeros(0).tensor([]) #.cuda()  # tensor([])
    bg_label = torch.tensor([0]).tensor([]) #.cuda()
