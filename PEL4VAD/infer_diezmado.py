...
def infer_func(model, dataloader, gt, logger, cfg):
    ...
    with torch.no_grad():
        ...
        for i, (v_input, name) in enumerate(dataloader):
            ...
            print(f"prediction time: {time_end:.4f} seconds")
            ...
            # save predictions in .npy fileg
            np.save(f"predictions/test/30fps/context_auto_minitest_20/{name}_pred.npy", pred_save)
        ...
        normal_labels = list(normal_labels.cpu().detach().numpy())
        normal_preds = list(normal_preds.cpu().detach().numpy())
        far = cal_false_alarm(normal_labels, np.repeat(normal_preds,16*rate))
        far2 = cal_false_alarm2(gt, np.repeat(pred, 16*rate))
    ...
    logger.info('offline AUC:{:.4f} AP:{:.4f} FAR_normal:{:.4f} FAR_all:{:.4f}| Complete in {:.0f}m {:.0f}s\n'.format(
        roc_auc, pr_auc, far, far2, time_elapsed // 60, time_elapsed % 60))
    logger.info(f"Full Prediction time for is : {time_elapsed:.4f} seconds")
