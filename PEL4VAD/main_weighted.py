...
def train(model, train_loader, test_loader, gt, logger):
    ...
    number_of_anomalies = 84148
    number_of_normal = 1025532
    total = number_of_anomalies + number_of_normal
    # weight_anomalies = total / (2 * number_of_anomalies)
    # weight_normal = total / (2 * number_of_normal)
    weight_anomalies = number_of_normal / number_of_anomalies
    weight = torch.tensor([weight_anomalies]).cuda()
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(['--dataset', 'ucf-crime', '--mode', 'train'])
