...
from train import train_func, freeze_encoder_layers
from test import test_func
from infer_diezmado import infer_func
import argparse
import copy
...
def load_checkpoint(model, ckpt_path, logger):
    if os.path.isfile(ckpt_path):
        ...
        weight_dict = torch.load(ckpt_path, weights_only=True)
        ...
        end_time_weight = time.time() - start_time
        logger.info(f"weights loading time: {end_time_weight:.4f} seconds")
    else:
...
def train(model, train_loader, test_loader, gt, logger):
    ...
    if args.finetuning == True:
        ...
        logger.info('Encoder layers are frozen!')
    ...
    torch.save(model.state_dict(), cfg.save_dir, 'fine_tuned_' + cfg.model_name + '_' + str(round(best_auc, 4)).split('.')[1] + '.pkl')
...
def main(cfg):
    ...
    test_loader = DataLoader(test_data, batch_size=cfg.test_bs, shuffle=False,
                             num_workers=cfg.workers, pin_memory=True)
    end_time_dataset = time.time() - start_time
    logger.info(f"dataset creation time: {end_time_dataset:.4f} seconds")
    # cuando se solo un .npy por archivo
    # test_loader = DataLoader(test_data, batch_size=1, shuffle=False,
    #                          num_workers=cfg.workers, pin_memory=True)
    
    time_start = time.time()
    model = XModel(cfg)
    time_end_model = time.time() - time_start
    logger.info(f"model creation time: {time_end_model:.4f} seconds")
    gt = np.load(cfg.gt)
...
if __name__ == '__main__':
    ...
    # set finetuning to True to freeze the encoder layers
    parser.add_argument('--finetuning', default=False, help='model status: (True or False)')
    # args = parser.parse_args()
    args = parser.parse_args(['--dataset', 'ucf-crime', '--mode', 'infer'])
