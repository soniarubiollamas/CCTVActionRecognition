...
def get_predict(test_loader, net):
    ...
    print(f"time to load data: {time.time()-time1}")
    frame_predict = []
    real_res = []
    total_frame_count = []
    ...
    for i in range(len(total_frame_count)-1):
        ...
        name = names[i]
        np.save(f"frame_label/predictions/test_normalized/30fps/local/{name}_pred.npy", archivo)
