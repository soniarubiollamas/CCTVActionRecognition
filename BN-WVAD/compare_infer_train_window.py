...
def plot_npy_files_gt(folder1, folder2, folder3, folder4, folder5, filename, label1, label2, label3, label4, ax):
    # Cargar los datos
    data1 = np.load(folder1)
    data2 = np.load(folder2)
    data3 = np.load(folder3)
    gt = np.load(folder4)
    original_gt = np.load(folder5)
    ...
    ax.plot(original_gt, label="original GT")
    ...
    # Ajustar los límites del gráfico
    max_ylim = max(max(data1), max(data2), max(data3), max(gt), max(original_gt))
    ax.set_ylim(0, 1.01)
    max_len = len(gt)
    ax.set_xlim(0, max_len)
...
for i, archivo in enumerate(archivos_seleccionados):
    ...
    gt = f'C:/Users/xoni/Documents/1_uni/1_insterships/cuda_version/PEL4VAD-master/predictions/gt/windows/{name}_pred.npy'
    original_gt = f'C:/Users/xoni/Documents/1_uni/1_insterships/cuda_version/PEL4VAD-master/predictions/gt/{name}_x264_pred.npy'
    ...
    if compare == True:
        PEL4VAd = f'C:/Users/xoni/Documents/1_uni/1_insterships/cuda_version/PEL4VAD-master/predictions/test/windows/PEL4VAD/{name}_pred.npy'
        URDMU = f'C:/Users/xoni/Documents/1_uni/1_insterships/cuda_version/PEL4VAD-master/predictions/test/windows/URDMU/{name}_pred.npy'
        BNWVAD = f'C:/Users/xoni/Documents/1_uni/1_insterships/cuda_version/PEL4VAD-master/predictions/test/windows/BNWVAD/{name}_pred.npy'
        ...
        if mode == "test":
            ...
            plot_npy_files_gt(PEL4VAd, URDMU, BNWVAD, gt, original_gt, filename, label1, label2, label3, label4="GT", ax=ax[i])
        else:
