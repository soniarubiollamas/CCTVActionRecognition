...
def load_and_concatenate_files(pred_folder, gt_folder, mode):
    ...
    for pred_file in os.listdir(gt_folder):
        ...
        if os.path.isfile(pred_path) and os.path.isfile(gt_path):
            ...
            # add this so that only the original preds of the prediction are used, and no the extended part
            context_len = (len(preds) - len(gt)) // 2
            preds = preds[context_len:len(preds)-context_len]
...
mode = "test_normalized"
pred_folder = f"frame_label/predictions/{mode}/30fps/context"
